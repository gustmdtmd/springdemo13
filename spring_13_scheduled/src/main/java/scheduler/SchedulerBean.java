...
로그(log) 프로그램 개발이나 운영시 문제점을 추척하거나 운영상태를 모니터하기 위한 텍스트이다.
System.out.println();을 사용하여 로그를 확인할 수 있으나 이보다는 로그를 기록하는 방법을 사용한다.

log4j은 다음과 같은 레벨은 갖는다.
...
WARNING(WARN) : 소프트웨어가 동작하고 있지만, 예상치 못한 일이 발생하고 가까운 미래에 문제가 발생할 수 있다는 경고이다.
               예로 디스크 공간 부족 -> 프로그램은 동작되지만 앞으로 문제가 발생할 수도 있다.
